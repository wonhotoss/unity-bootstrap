Shader "UI Extensions/Particles/Additive no maintex" {
Properties {
	_TintColor ("Tint Color", Color) = (0.5,0.5,0.5,0.5)
	no_MainTex ("Particle Texture", 2D) = "white" {}
	...
	SubShader {
		...
		Pass {
			...
			v2f vert (appdata_t IN)
			{
				...
				#endif
				v.color = IN.color;
				v.texcoord = TRANSFORM_TEX(IN.texcoord,no_MainTex);
			...
			fixed4 frag (v2f IN) : SV_Target
			{
				...
				fixed4 col = 2.0f * IN.color * _TintColor * tex2D(no_MainTex, IN.texcoord);
